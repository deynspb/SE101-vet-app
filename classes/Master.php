...
Class Master extends DBConnection {
	...
	function update_appointment_status(){
		extract($_POST);
		error_log("Received id: $id, status: $status"); // Log received parameters
		
		$sql = "UPDATE `appointment_list` SET `status` = '{$status}' WHERE `id` = '{$id}'";
		error_log("SQL Query: $sql"); // Log the SQL query
		
		$update = $this->conn->query($sql);
		if($update){
			$resp['status'] = 'success';
			$resp['message'] = "Appointment request status has been successfully updated.";
		} else {
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
			error_log("SQL Error: " . $this->conn->error); // Log any SQL errors
		}
		return json_encode($resp);
	}
